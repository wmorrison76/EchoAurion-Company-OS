...
Runtime encoding: `src/lib/constitution.ts` (version `1.0.0`).
...
| `pr_only_code` | PR-only code | Architect produces draft plan / draft GitHub PR. **Merge is human/CI.** |
...
| `standby` | Auto-answer low-risk TEXT; execute soft outbox tools | Paid execute, merge, T3+ auto, RDP |
| `autopilot` | Same as standby + more stub directives | Merge, paid execute, T3+ auto, RDP |
